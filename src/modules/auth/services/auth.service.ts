import { DEFAULT_OTP_EXPIRY_TIME, FIVE_MINUTES, INDIA_COUNTRY_CODE, MAX_OTP_ATTEMPTS, MAX_OTP_VALUE, MIN_OTP_VALUE, OTP_QUEUE, REFRESH_TOKEN, SEND_OTP, TOKEN_SYNC_QUEUE } from "@/common/constants";
import { REDIS_CLIENT } from "@/redis-v2/redis-v2.module";
import { User } from "@/schema/user";
import { createCryptoHash, GenerateOtp, generateToken, validateHash } from "@/utils";
...
export class AuthService {
    ...
    public async sendOtp(body: SendOtpDto) {
        ...
        if (existing) {
            // can create mapper to send to the FE to handle things easily
    ...
    public async verifyOtp(phoneNumber: string, otp: string) {
        ...
        const verificationTimeDifference = otpValidationTime - Number(otpGenerationTime);
        ...
        if (parsedOtp.attempts >= MAX_OTP_ATTEMPTS) {
            ...
            throw new UnauthorizedException('Too many attempts, Please request a new OTP', { cause: 'Too Many Invalid Attemps', description: 'AUTHORIZATION FAILED' })
        }
        ...
        if (!isValid) {
            ...
            throw new UnauthorizedException(
                `Invalid OTP. ${MAX_OTP_ATTEMPTS - parsedOtp.attempts} attempts remaining`
            );
        }
        ...
        const accessTokenSecret = this.configService.get<string>('SECRET.ACCESS_TOKEN')!;
        const refreshTokenSecret = this.configService.get<string>('SECRET.REFRESH_TOKEN')!;
        ...
        const { accessToken, refreshToken } = generateToken(payLoad, accessTokenSecret, refreshTokenSecret)

        await this.tokenSyncQueue.add(REFRESH_TOKEN, { userId: user._id, refreshToken })
